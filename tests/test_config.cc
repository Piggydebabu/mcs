...
mcs::ConfigVar<float>::ptr g_float = 
    mcs::Config::Lookup("global.float", (float)10.2f, "global float");

// 字符串需显示构造，不能传字符串常量
mcs::ConfigVar<std::string>::ptr g_string =
    mcs::Config::Lookup("global.string", std::string("helloworld"), "global string");

mcs::ConfigVar<std::vector<int>>::ptr g_int_vec = 
    mcs::Config::Lookup("global.int_vec", std::vector<int>{1, 2, 3}, "global int vec");

mcs::ConfigVar<std::list<int>>::ptr g_int_list = 
    mcs::Config::Lookup("global.int_list", std::list<int>{1, 2, 3}, "global int list");

mcs::ConfigVar<std::set<int>>::ptr g_int_set = 
    mcs::Config::Lookup("global.int_set", std::set<int>{1, 2, 3}, "global int set");

mcs::ConfigVar<std::unordered_set<int>>::ptr g_int_unordered_set = 
    mcs::Config::Lookup("global.int_unordered_set", std::unordered_set<int>{1, 2, 3}, "global int unordered_set");

mcs::ConfigVar<std::map<std::string, int>>::ptr g_map_string2int = 
    mcs::Config::Lookup("global.map_string2int", std::map<std::string, int>{{"key1", 1}, {"key2", 2}}, "global map string2int");

mcs::ConfigVar<std::unordered_map<std::string, int>>::ptr g_unordered_map_string2int = 
    mcs::Config::Lookup("global.unordered_map_string2int", std::unordered_map<std::string, int>{{"key1", 1}, {"key2", 2}}, "global unordered_map string2int");
...
// 实现自定义配置的YAML序列化与反序列化，这部分要放在mcs命名空间中
namespace mcs {
...
mcs::ConfigVar<Person>::ptr g_person = 
    mcs::Config::Lookup("class.person", Person(), "system person");

mcs::ConfigVar<std::map<std::string, Person>>::ptr g_person_map = 
    mcs::Config::Lookup("class.map", std::map<std::string, Person>(), "system person map");

mcs::ConfigVar<std::map<std::string, std::vector<Person>>>::ptr g_person_vec_map = 
    mcs::Config::Lookup("class.vec_map", std::map<std::string, std::vector<Person>>(), "system vec map");
