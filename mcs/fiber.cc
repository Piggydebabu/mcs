...
//     struct ucontext_t *uc_link;  // 指向另一个ucontext结构体的指针，通常用于连接多个上下文，例如协程的调用栈
//     stack_t uc_stack;         //栈字段，包含栈的起始地址，栈大小，栈指针
//     mcontext_t uc_mcontext;   // 程序的执行上下文字段
//     sigset_t uc_sigmask;
...
void Fiber::reset(std::function<void()> cb) {
    ...
    m_ctx.uc_stack.ss_sp   = m_stack;
    m_ctx.uc_stack.ss_size = m_stacksize;
    // makecontext(ucontext_t *ucp, void (*func)(void), int argc, ...): 在 ucp 指向的上下文中创建一个新的执行环境，
    // 其中 func 是新上下文的入口函数，argc 和后续参数是传递给 func 的参数。
    makecontext(&m_ctx, &Fiber::MainFunc, 0);
    m_state = READY;
}
...
void Fiber::MainFunc() {
    ...
    // 获取当前协程智能指针的原始指针，但不拥有所有权。
    // 重置智能指针 cur，减少引用计数，如果这是最后一个引用，则 Fiber 对象将被销毁。
    // 调用 yield 函数让出执行权，将控制权交还给之前执行的协程。这里使用 raw_ptr 是因为 cur 已经被重置，不再拥有 Fiber 对象的引用。
