...
public:
     ...
     * @return 是否处理成功
     */
    virtual int32_t handle(mcs::http::HttpRequest::ptr request
                   , mcs::http::HttpResponse::ptr response
                   , mcs::http::HttpSession::ptr session) = 0;
...
public:
    ...
    /// 函数回调类型定义
    typedef std::function<int32_t (mcs::http::HttpRequest::ptr request
                   , mcs::http::HttpResponse::ptr response
                   , mcs::http::HttpSession::ptr session)> callback;
    ...
    virtual int32_t handle(mcs::http::HttpRequest::ptr request
                   , mcs::http::HttpResponse::ptr response
                   , mcs::http::HttpSession::ptr session) override;
private:
    /// 回调函数
    callback m_cb;
};
